...
out1 =  tf.add(tf.matmul(x_data,A1), b1)
...
loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels = y_trgt, logits = out1))
init = tf.global_variables_initializer()
sess.run(init)


regularizer = tf.nn.l2_loss(A1)
...
prediction = tf.round(tf.sigmoid(out1)) # obviously round to {0,1}
